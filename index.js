...
app.get("/medicos/especialidade/:id", async (req, res) => {
  const idEspecialidade = req.params.id;

  await db.read();

  const filtro = db.data.medicos.filter((iten)=> iten.especialidade.id == parseInt(idEspecialidade));
...
app.get("/agendas/medico/:id", async (req, res) => {
  const idMedico = req.params.id;

  await db.read();

  const filtro = db.data.agendas.filter((iten)=> iten.medico.id == parseInt(idMedico));
